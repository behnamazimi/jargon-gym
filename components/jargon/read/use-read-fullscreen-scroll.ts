...
export function useReadFullscreenScroll(queue: ReadQueue) {
  ...
  // mounted card's IntersectionObserver (keyed on `onExposed`) would tear
  // down and recreate on every scroll-driven re-render, and a freshly
  // re-created observer re-evaluates intersection immediately — re-firing
  // exposure for a card that already fired moments earlier.
  const queueRef = useRef(queue);
  queueRef.current = queue;

  const registerCardNode = useCallback((termId: string, node: HTMLDivElement) => {
    cardNodesRef.current.set(termId, node);
    ...
    const currentTerm = queueRef.current.terms[queueRef.current.currentIndex];
    if (currentTerm?.id !== termId) return;
    hasScrolledToInitialRef.current = true;
    node.scrollIntoView({ behavior: "instant", block: "start" });
  }, []);

  const unregisterCardNode = useCallback((termId: string) => {
    cardNodesRef.current.delete(termId);
  }, []);
  ...
  const handleMarkedKnown = useCallback(async () => {
    const nextIndex = await queueRef.current.goNext();
